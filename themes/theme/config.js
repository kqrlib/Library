const CONFIG = {
  SIMPLE_LOGO_IMG: "/Logo.webp",
  SIMPLE_TOP_BAR: false, // Disable top bar as per requirements
  SIMPLE_TOP_BAR_CONTENT: process.env.NEXT_PUBLIC_THEME_SIMPLE_TOP_TIPS || "",
  SIMPLE_LOGO_DESCRIPTION:
    process.env.NEXT_PUBLIC_THEME_SIMPLE_LOGO_DESCRIPTION ||
    "<div>编程爱好者<br/>/互联网从业者<br/>/知识分享博主</div>",

  SIMPLE_AUTHOR_LINK: process.env.NEXT_PUBLIC_AUTHOR_LINK || "#",

  SIMPLE_POST_AD_ENABLE: process.env.NEXT_PUBLIC_SIMPLE_POST_AD_ENABLE || false, // 文章列表是否插入广告

  SIMPLE_POST_COVER_ENABLE: process.env.NEXT_PUBLIC_SIMPLE_POST_COVER_ENABLE || true, // Enable post covers by default

  SIMPLE_ARTICLE_RECOMMEND_POSTS: process.env.NEXT_PUBLIC_SIMPLE_ARTICLE_RECOMMEND_POSTS || true, // 文章详情底部显示推荐

  // Menu configuration
  SIMPLE_MENU_CATEGORY: true, // 显示分类
  SIMPLE_MENU_TAG: true, // 显示标签
  SIMPLE_MENU_ARCHIVE: true, // 显示归档
  SIMPLE_MENU_SEARCH: true, // 显示搜索

  // New configuration options
  SIMPLE_HEADER_STICKY: true, // Enable sticky header
  SIMPLE_ENABLE_ROUNDED_CORNERS: true, // Enable rounded corners for cards
  SIMPLE_ENABLE_CARD_SHADOWS: true, // Enable shadows for cards
}
export default CONFIG
